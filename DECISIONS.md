...
## Ciclo 19 (17/09, commit 3d3a10a) — NavBar vídeos + transição
1. PEDIDO DO EDUARDO (17/09 manhã): M da transição centralizado e maior; vídeos dos submenus da NavBar "muito ruins, fora de proporção e alguns sem sentido com o conteúdo".
2. TRANSIÇÃO: o M ficava no canto superior esquerdo porque o GSAP seta display:block inline no container, matando o display:flex do CSS (centralização nunca aplicava). Corrigido com position:absolute + translate:-50% -50% (propriedade CSS separada do transform do gsap) e width 72-132px (era 30-48). Validado ao vivo: cx=720 (centro exato em 1440), w=125.
3. VÍDEOS: os 7 nv_* re-encodados DIRETO das fontes 4K do drive_src (proveniência confirmada por phash com filmstrip fps=4): CRF 21, 960x540, faststart — 324-603KB (~2x bitrate, era ~225kbps/141-375KB).
4. PROPORÇÃO: painel do submenu forçava 4:3 + object-fit:cover sobre fontes 16:9 (cortava 25% da largura) → agora 16:9; thumb do menu mobile 76x52 → 76x43.
5. CONTEÚDO: slot Desenvolvimento mostrava nv_placa (placa da marca na natureza = conteúdo de marca/design, não dev) → novo nv_dev.mp4 cortado do Clipe_3_sistema_digital (sistema digital/telas = desenvolvimento). Demais slots confirmados temáticos por phash: Sistemas=camadas, Design=planos, Automação=dados, Sobre=reel, Tecnologia=dolly, Diagnóstico=gráficos.
6. AUDITORIA (16 rotas mobile + 15 desktop): 0 overflow, 0 erros JS; único achado real = chips de filtro do /work mobile <38px → min-height 44px. (Nota de método: threshold fixo do script deu falso "overflow 1440" no desktop — é a largura da viewport, não scroll.)
